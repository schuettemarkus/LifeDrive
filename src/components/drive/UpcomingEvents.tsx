...
export function UpcomingEvents({ initialEvents }: { initialEvents?: UpcomingEvent[] }) {
  ...
  // Only today's events whose end time is still in the future. Keep LifeDrive
  // focus blocks alongside real events so the user can tap them to open the
  // underlying task.
  const visible = useMemo(() => {
    if (!events) return [];
    const endOfToday = new Date();
    endOfToday.setHours(23, 59, 59, 999);
    const todayCap = endOfToday.getTime();
    return events
      .filter((e) => {
        const endMs = new Date(e.end).getTime();
        const startMs = new Date(e.start).getTime();
        return endMs > now && startMs <= todayCap;
      })
      ...
      {loading ? (
        ...
        <GlassCard inset variant="subtle">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm text-white/70">
              Nothing left on today's calendar. Free skies the rest of the day.
            </p>
          </div>
        </GlassCard>
      ) : (
        <div className="space-y-2">
          {visible.map((ev, i) => {
            ...
            const linkable = Boolean(ev.itemId);
            const card = (
              <GlassCard
                inset
                ...
                <div className="relative flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5">
                      ...
                      {ev.isLifeDrive && (
                        <span className="rounded-pill border border-white/15 bg-white/5 px-2 py-0.5 text-[10px] uppercase tracking-wider text-white/65">
                          focus
                        </span>
                      )}
                    </div>
            ...
            return (
              ...
              >
                {linkable ? (
                  <Link href={`/item/${ev.itemId}`} className="block no-tap-highlight">
                    {card}
                  </Link>
                ) : (
                  card
